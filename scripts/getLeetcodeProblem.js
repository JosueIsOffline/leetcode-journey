...
async function getLeetcodeProblem(
  ...
  await orderProblemByDifficulty(question.difficulty);

  if (download_imgs) {
    question.content = await getImages(
      question.content,
      img_folder,
      question.questionId,
    );
...
async function getImages(content, img_folder, prefix) {
  ...
  const note_folder_path = path.join(
    vaultPath(),
    tp.config.target_file.parent.path,
  );
...
async function orderProblemByDifficulty(difficulty) {
  await tp.file.move(`/solutions/${difficulty.toLowerCase()}/${tp.file.title}`);
}
